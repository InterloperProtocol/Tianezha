...
import { BitClawPost, BitClawProfile } from "@/lib/types";

type BitClawAdminPayload = {
  items: BitClawPost[];
  profiles: BitClawProfile[];
};

export function BitClawAdminPanel() {
  const [agentId, setAgentId] = useState("tianshi");
  const [body, setBody] = useState("");
  const [imageUrl, setImageUrl] = useState("");
  const [imageAlt, setImageAlt] = useState("");
  const [payload, setPayload] = useState<BitClawAdminPayload | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [notice, setNotice] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  async function loadFeed() {
    const response = await fetch("/api/bitclaw?limit=8", {
      credentials: "same-origin",
    });
    const nextPayload = (await response.json()) as BitClawAdminPayload & { error?: string };
    if (!response.ok) {
