...
import { POST } from "@/app/api/internal-admin/goonbook/posts/[postId]/hide/route";

describe("/api/internal-admin/goonbook/posts/[postId]/hide", () => {
  beforeEach(() => {
    goonBookModule.hideGoonBookPost.mockReset();
  ...
  it("returns 401 when the owner is not authenticated", async () => {
    ...
    const request = new NextRequest("https://example.com/api/internal-admin/goonbook/posts/post-1/hide", {
      method: "POST",
  ...
  it("hides the post for an authenticated owner", async () => {
    ...
    goonBookModule.hideGoonBookPost.mockResolvedValue({
      id: "post-1",
      isHidden: true,
    });
    ...
    expect(goonBookModule.hideGoonBookPost).toHaveBeenCalledWith({
      adminUsername: "owner",
      postId: "post-1",
      reason: "hide",
    });
