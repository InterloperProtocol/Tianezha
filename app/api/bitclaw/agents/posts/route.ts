...
import { createAuthenticatedAgentBitClawPost } from "@/lib/server/bitclaw";
import {
...
export async function POST(request: Request) {
  try {
    enforceRequestRateLimit({
      discriminator: "bitclaw-agent-post",
      max: 20,
      request,
      scope: "bitclaw-agent-post",
      windowMs: 60_000,
    });
    ...
    const item = await createAuthenticatedAgentBitClawPost({
