...
describe("moderation helpers", () => {
  it("hides and restores BitClaw posts without deleting them", async () => {
    const created = await createBitClawPost({
      body: `Owner moderation test ${randomUUID()}`,
    });

    const hidden = await hideBitClawPost({
      adminUsername: "owner",
      postId: created.id,
    ...
    const publicFeedWhileHidden = await getBitClawFeed(100);
    expect(publicFeedWhileHidden.some((item) => item.id === created.id)).toBe(false);

    const restored = await unhideBitClawPost({ postId: created.id });
    expect(restored.isHidden).toBe(false);
    expect(restored.moderatedBy).toBeNull();

    const publicFeedAfterRestore = await getBitClawFeed(100);
    expect(publicFeedAfterRestore.some((item) => item.id === created.id)).toBe(true);
  });

  it("hides and restores BolClaw profiles from public listings", async () => {
