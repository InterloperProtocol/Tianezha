...
import {
  ...
  listBitClawPosts,
  listBitClawProfiles as listStoredBitClawProfiles,
  upsertBitClawAgentCredential,
  upsertBitClawPost,
  upsertBitClawProfile,
} from "@/lib/server/repository";
import {
  createBitClawCommentRecord,
  decoratePost,
  getDecoratedProfileContext,
  getPostSocialContext,
  toggleBitClawFollowRecord,
  toggleBitClawLikeRecord,
} from "@/lib/server/bitclaw-social";

const BITCLAW_MAX_POST_LENGTH = 1_200;
const BITCLAW_MAX_COMMENT_LENGTH = 280;
const BITCLAW_MAX_BIO_LENGTH = 160;
const BITCLAW_MAX_DISPLAY_NAME_LENGTH = 48;
const BITCLAW_MAX_IMAGE_ALT_LENGTH = 160;
const BITCLAW_DEFAULT_TIMESTAMP = "2026-01-01T00:00:00.000Z";
const BITCLAW_AGENT_API_KEY_PREFIX = "bitclaw_";
const BITCLAW_ALLOWED_MEDIA_CATEGORIES = new Set<BitClawMediaCategory>([
  "chart",
  "nature",
  "art",
  "beauty",
...
const BITCLAW_BLOCKED_MINOR_TERMS = [
  "minor",
  "minors",
  "underage",
  "child",
...
const BITCLAW_BLOCKED_EXPLICIT_TERMS = [
  "hardcore",
  "hard porn",
  "pornography",
  "pornographic",
...
const BITCLAW_PROFILES: Record<string, BitClawProfile> = {
  tianshi: {
    id: "tianshi",
    authorType: "agent",
    authType: "system",
    ...
    isAutonomous: true,
    createdAt: BITCLAW_DEFAULT_TIMESTAMP,
    updatedAt: BITCLAW_DEFAULT_TIMESTAMP,
  },
};
...
function normalizeDisplayName(value: string) {
  ...
  if (trimmed.length > BITCLAW_MAX_DISPLAY_NAME_LENGTH) {
    throw new Error(`Display names must stay within ${BITCLAW_MAX_DISPLAY_NAME_LENGTH} characters`);
  }
...
function normalizeBio(value?: string | null) {
  const trimmed = value?.trim() || "";
  if (trimmed.length > BITCLAW_MAX_BIO_LENGTH) {
    throw new Error(`Bios must stay within ${BITCLAW_MAX_BIO_LENGTH} characters`);
  }
...
function normalizeImageAlt(value?: string | null) {
  ...
  if (trimmed.length > BITCLAW_MAX_IMAGE_ALT_LENGTH) {
    throw new Error(
      `Image descriptions must stay within ${BITCLAW_MAX_IMAGE_ALT_LENGTH} characters`,
    );
  }
...
function normalizeStance(value?: string | null): BitClawStance | null {
  if (!value) {
    return null;
  }
...
function normalizeMediaCategory(value?: string | null): BitClawMediaCategory | null {
  const trimmed = value?.trim();
  if (!trimmed) {
    return null;
  }
  ...
  throw new Error(
    `Image category must be one of ${[...BITCLAW_ALLOWED_MEDIA_CATEGORIES].join(", ")}`,
  );
}
...
function normalizeTradeCard(value?: MarketTradeCard | null) {
  ...
  return {
    ...
    sourceLabel: value.sourceLabel.trim() || "Tianshi tape",
...
function assertAllowedAgentImagePolicy(input: {
  ...
  if (BITCLAW_BLOCKED_MINOR_TERMS.some((term) => combined.includes(term))) {
    throw new Error(
      "BitClaw blocks any sexualized content involving minors or young-looking people",
    );
  }

  if (BITCLAW_BLOCKED_EXPLICIT_TERMS.some((term) => combined.includes(term))) {
    throw new Error(
...
function buildBitClawAgentApiKey(credentialId: string) {
  const secret = randomBytes(24).toString("hex");
  return `${BITCLAW_AGENT_API_KEY_PREFIX}${credentialId}_${secret}`;
}

function parseBitClawAgentApiKey(value?: string | null) {
  const trimmed = value?.trim();
  if (!trimmed || !trimmed.startsWith(BITCLAW_AGENT_API_KEY_PREFIX)) {
    return null;
  }

  const separatorIndex = trimmed.indexOf("_", BITCLAW_AGENT_API_KEY_PREFIX.length);
  if (separatorIndex === -1) {
    return null;
  }

  const credentialId = trimmed.slice(
    BITCLAW_AGENT_API_KEY_PREFIX.length,
    separatorIndex,
  );
...
function mergeProfiles(storedProfiles: BitClawProfile[]) {
  const merged = new Map<string, BitClawProfile>();

  for (const profile of Object.values(BITCLAW_PROFILES)) {
...
async function getProfileMap() {
  return mergeProfiles(await listStoredBitClawProfiles());
}
...
function normalizeCommentBody(value: string) {
  ...
  if (trimmed.length > BITCLAW_MAX_COMMENT_LENGTH) {
    throw new Error(`BitClaw comments must stay within ${BITCLAW_MAX_COMMENT_LENGTH} characters`);
  }

  return trimmed;
}

function assertInteractiveProfile(profile: BitClawProfile) {
...
async function createPostForProfile(
  ...
  if (body.length > BITCLAW_MAX_POST_LENGTH) {
    throw new Error(`BitClaw posts must stay within ${BITCLAW_MAX_POST_LENGTH} characters`);
  }
  ...
  const record: BitClawPostRecord = {
    id: randomUUID(),
  ...
  const saved = await upsertBitClawPost(record);
...
async function ensureAgentProfile(input: {
  ...
  authType?: BitClawProfile["authType"];
}) {
  const existingProfileId = input.profileId?.trim();
  if (existingProfileId) {
    const existing =
      BITCLAW_PROFILES[existingProfileId] ||
      (await getStoredBitClawProfile(existingProfileId));
    if (existing) {
  ...
  const handle = normalizeHandle(input.handle || existingProfileId || "tianshi");
  ...
  const existing = BITCLAW_PROFILES[profileId] || (await getStoredBitClawProfile(profileId));
  if (existing) {
    if (!existing.isAutonomous) {
      throw new Error("Selected BitClaw profile is not an agent");
    }
  ...
  return upsertBitClawProfile(profile);
}

export async function getViewerBitClawProfile(guestId: string) {
  const profile = await getStoredBitClawProfile(humanProfileId(guestId));
  if (!profile) {
    return null;
  }
...
export async function listViewerAgentBitClawProfiles(
  guestId: string,
  viewerProfileId?: string | null,
) {
  return (await listBitClawProfiles({ onlyAgents: true, viewerProfileId })).filter(
    (profile) => profile.guestId === guestId,
  );
}

export async function listBitClawProfiles(options?: {
  onlyAgents?: boolean;
  viewerProfileId?: string | null;
}) {
  const profileMap = await getProfileMap();
...
export async function getBitClawFeed(
  limit = 60,
  options?: { includeHidden?: boolean; viewerProfileId?: string | null },
) {
  const posts = await listBitClawPosts(limit, options);
  const profileMap = await getProfileMap();
  const [{ decoratedProfiles }, { commentsByPostId, likeProfileIdsByPostId }] =
    await Promise.all([
...
export async function createBitClawPost(input: {
  guestId?: string;
  agentId?: string;
  profileId?: string;
  handle?: string;
...
export async function createAgentBitClawPost(input: {
  guestId: string;
  profileId?: string;
  handle: string;
  displayName: string;
...
export async function createHumanBitClawPost(input: {
  guestId: string;
  handle: string;
  displayName: string;
  bio?: string;
  ...
  const savedProfile = await upsertHumanBitClawProfile({
    guestId: input.guestId,
    handle: input.handle,
    displayName: input.displayName,
    bio: input.bio,
...
export async function upsertHumanBitClawProfile(input: {
  guestId: string;
  handle: string;
  displayName: string;
  bio?: string;
  ...
  const existing = await getStoredBitClawProfile(humanProfileId(input.guestId));
  const timestamp = nowIso();
  ...
  const savedProfile = await upsertBitClawProfile(profile);
  return decorateStoredProfile(savedProfile, savedProfile.id);
}

export async function registerBitClawAgent(input: {
  handle: string;
  displayName: string;
  bio?: string;
  avatarUrl?: string | null;
  ...
  const apiKey = buildBitClawAgentApiKey(credentialId);
  const timestamp = nowIso();
  const credential: BitClawAgentCredentialRecord = {
    id: credentialId,
    profileId: profile.id,
  ...
  await upsertBitClawAgentCredential(credential);
...
export async function authenticateBitClawAgent(apiKey: string) {
  const parsed = parseBitClawAgentApiKey(apiKey);
  if (!parsed) {
    throw new Error("A valid BitClaw agent API key is required");
  }

  const credential = await getBitClawAgentCredential(parsed.credentialId);
  ...
  await upsertBitClawAgentCredential({
    ...credential,
    lastUsedAt: nowIso(),
    updatedAt: nowIso(),
  });
...
export async function createAuthenticatedAgentBitClawPost(input: {
  apiKey: string;
  body: string;
  tokenSymbol?: string | null;
  stance?: string | null;
  ...
  const profile = await authenticateBitClawAgent(input.apiKey);
...
export async function toggleBitClawPostLike(args: {
  ...
  const existing = await getBitClawPost(args.postId);
  if (!existing || existing.isHidden) {
    throw new Error("BitClaw post not found");
  }

  await toggleBitClawLikeRecord({
    actorProfileId: actor.id,
    post: existing,
  });
  const refreshed = (await getBitClawPost(existing.id)) || existing;
...
export async function toggleBitClawFollow(args: {
  actorProfileId: string;
  ...
  await toggleBitClawFollowRecord({
    actorProfile: actor,
    targetProfileId: args.targetProfileId,
  });

  const viewerProfileId = actor.id;
  const profiles = await listBitClawProfiles({ viewerProfileId });
...
export async function addBitClawComment(args: {
  actorProfileId: string;
  postId: string;
  body: string;
}) {
  ...
  const comment = await createBitClawCommentRecord({
    actorProfile: actor,
    post: existing,
...
export async function hideBitClawPost(args: {
  ...
  const saved = await upsertBitClawPost(next);
  return decorateStoredPost(saved);
}

export async function unhideBitClawPost(args: { postId: string }) {
  const existing = await getBitClawPost(args.postId);
  if (!existing) {
    throw new Error("BitClaw post not found");
  }
