...
function getTelegramConfig() {
  ...
  return {
    token: env.TIANSHI_TELEGRAM_BOT_TOKEN,
    chatId: env.TIANSHI_TELEGRAM_CHAT_ID,
    threadId: env.TIANSHI_TELEGRAM_THREAD_ID,
    description: env.TIANSHI_TELEGRAM_DESCRIPTION,
    shortDescription: env.TIANSHI_TELEGRAM_SHORT_DESCRIPTION,
  };
}
...
function formatEventMessage(event: AutonomousFeedEvent) {
  const lines = [
    `<b>Tianshi ${escapeHtml(event.kind)}</b>`,
...
export function isTianshiTelegramBroadcastEnabled() {
  return canUseTelegram();
}

export async function bootstrapTianshiTelegramBot() {
  const { token } = getTelegramConfig();
  if (!token) {
    return false;
  }
